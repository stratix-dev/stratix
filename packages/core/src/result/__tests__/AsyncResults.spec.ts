...
describe('AsyncResults', () => {
  ...
  describe('flatMap', () => {
    it('should chain successful results', async () => {
      ...
      const chained = await AsyncResults.flatMap(result, async (n) => Success.create(n * 2));
    ...
    it('should propagate failure from chained operation', async () => {
      ...
      const chained = await AsyncResults.flatMap(result, async () => Failure.create(chainError));
  ...
  describe('real-world scenarios', () => {
    ...
    it('should chain multiple async operations', async () => {
      ...
      const loadUser = async (id: string) => Success.create({ id, name: 'John' });
      const formatUser = async (user: { id: string; name: string }) =>
        Success.create(`${user.name} (${user.id})`);

      const result = await AsyncResults.flatMap(
        getUserId(),
        async (userId) => await AsyncResults.flatMap(loadUser(userId), formatUser)
      );
