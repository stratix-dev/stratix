import { Result, Success, Failure } from '../shared/result/Result.js';
...
export const Validators = {
  ...
  notEmpty(value: string, fieldName: string = 'Field'): Result<string, DomainError> {
    if (!value || value.trim().length === 0) {
      return Failure.create(new DomainError('EMPTY_VALUE', `${fieldName} cannot be empty`));
    }
  ...
  length(
    ...
    if (min !== undefined && trimmed.length < min) {
      return Failure.create(
        new DomainError('TOO_SHORT', `${fieldName} must be at least ${min} characters`)
      );
    }
  ...
  range(
    ...
    if (min !== undefined && value < min) {
      return Failure.create(new DomainError('BELOW_MIN', `${fieldName} must be at least ${min}`));
    }

    if (max !== undefined && value > max) {
      return Failure.create(new DomainError('ABOVE_MAX', `${fieldName} cannot exceed ${max}`));
    }
  ...
  email(value: string): Result<string, DomainError> {
    return Validators.pattern(value, /^[^\s@]+@[^\s@]+\.[^\s@]+$/, 'Invalid email format');
  },
