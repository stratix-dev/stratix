...
import { Logger, LoggerConfig, LogTransport } from '../../core/ports/Logger.js';
...
export class ConsoleLoggerFactory {
  ...
  create(context?: string): Logger {
    return new ConsoleLoggerAdapter({ config: this.config, transports: this.transports, context });
  }

  /**
   * Crea un child logger con contexto adicional
   */
  child(parent: ConsoleLoggerAdapter, childContext: string): Logger {
